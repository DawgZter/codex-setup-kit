---
name: improve
description: 'Explicit-only codebase advisory workflow. Invoke with `$improve` to survey a codebase and produce prioritized, self-contained implementation plans for other agents. It remains read-only on source code and may cover bugs, security, performance, tests, debt, migrations, DX, roadmap, or product direction. Do not auto-trigger from ordinary requests to improve or audit code.'
